...
#SET UP DRINKS MENU
    ...
    @soft_drink = Drink.new(2, soft_drink_name, 0)
    @pint = Drink.new(4, pint_name, 3)
    @hard_liquour = Drink.new(7, hard_liquour_name, 5)
...
#REFACTOR THIS TO USE .each IN YOUR COPIOUS FREE TIME
  ...
  name_array = ["Matt", "Ally", "Eoghan", "Richard", "Nick", "Gill", "Marilena", "Irma", "Louise", "Lewis", "Alex", "Charlie", "Johnny", "Mike", "Chris", "Daniel", "Paul", "Mungo", "Simon", "Craig", "Sandy", "Matthew"]
  ...
  guest_array = name_array.map{|guest| Guest.new({
    "name" => guest,
    "money" => ((d10.shuffle.first + 5) * 10),
    "favourite_song" => @song_library.shuffle.first,
    "favourite_drink" => @drinks_array.shuffle.first
    })}
...
#CREATE STARTING CONDITIONS FOR GAME
  ...
  def run
    not_yet_in_play_guests = @remaining_guests

    while @room_1.playlist[0].title != "Bohemian Rhapsody"
      @viewer.game_turn_option(@office, @room_1, @drinks_array)
      @room_1.take_turn
      @viewer.summarise_turn_conditions(@office, @room_1)
...
#HOW ARE DUPLICATE PEOPLE GETTING IN THE MIX?
      ...
      guests_joining_queue = not_yet_in_play_guests.shuffle.pop(number_to_join_queue)
